...
const Offer2 = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const new_data = {
      [question.audience]: data[question.audience],
      [question.painpoint]: data[question.painpoint],
      [question.provide]: data[question.provide],
      [question.improve]: check,
    }
    if (
      !check ||
      !data[question.audience] ||
      !data[question.painpoint] ||
      !data[question.provide]
    ) {
    ...
    <div>
      <div className="flex flex-col">
        <div className="text-sm font-[600] text-center m-auto bg-[#6FF] justify-center fit-content w-40 px-4 py-2 rounded-[16px]">
          <label>Offer Ends Soon!</label>
        </div>
        <div className="flex flex-col mt-2 mb-2 gap-y-2">
          <div className="flex justify-between m-auto text-center w-50">
            <div className="flex flex-col">
              <label id="day" className="text-[16px] font-[500]">
                {leftTime.days}
              </label>
              <label className="text-[10px] font-[500] text-[#0176CC]">
                DAYS
              </label>
            </div>
            <div className="flex flex-col">
            ...
            <div className="flex flex-col">
              <label id="hour" className="text-[16px] font-[500]">
                {leftTime.hours}
              </label>
              <label className="text-[10px] font-[500] text-[#0176CC]">
                HOURS
            ...
            <div className="flex flex-col">
              <label id="minute" className="text-[16px] font-[500]">
                {leftTime.mins}
              </label>
              <label className="text-[10px] font-[500] text-[#0176CC]">
                MINUTES
            ...
            <div className="flex flex-col">
              <label id="second" className="text-[16px] font-[500]">
                {leftTime.secs}
              </label>
              <label className="text-[10px] font-[500] text-[#0176CC]">
                SECONDS
      ...
      <div className="relative flex flex-col items-center px-1 py-10 m-auto bg-white gap-y-2 rounded-2xl">
        ...
        <form className="flex flex-col px-[32px] " onSubmit={handleSubmit}>
          ...
          <input
            className="bg-[#0174c8] cursor-pointer py-3 mt-3 rounded-full text-slate-100 hover:bg-[#4dabf7]"
            type="submit"
            value="Try for FREE"
          />
        </form>
      ...
      {showSignUp && (
        <SignUp clicked={true} data={formData} removeSignUp={setShowSignUp} />
      )}
      {showSignIn && <SignIn />}
      {showOtp && <Otp />}
    </div>
